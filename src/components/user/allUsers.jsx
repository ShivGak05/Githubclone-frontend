...
const AllUsers = () => {
  ...
  try {
    const result = await fetch("http://localhost:3000/user/all");
...
useEffect(() => {
  const fetchCurrentUser = async () => {
    try {
      const res = await fetch(`http://localhost:3000/user/${currId}`);
...
const handleFollowToggle = async (targetUserId) => {
  try {
    const res = await fetch(`http://localhost:3000/user/follow/${currId}/${targetUserId}`, {
      method: "PATCH",
    });
    const data = await res.json();
